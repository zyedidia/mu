...
func NewNode(Kind SplitType, x, y, w, h int, parent *Node, id uint) *Node {
	n := new(Node)
	n.Kind = Kind
	n.canResize = true
	n.propScale = true
...
func (n *Node) GetNode(id uint) *Node {
	if n.id == id && n.IsLeaf() {
		return n
	}
...
func (n *Node) vVSplit(right bool) uint {
	ind := 0
	for i, c := range n.parent.children {
		if c.id == n.id {
			ind = i
...
func (n *Node) hHSplit(bottom bool) uint {
	ind := 0
	for i, c := range n.parent.children {
		if c.id == n.id {
			ind = i
...
func (n *Node) vHSplit(i int, right bool) uint {
	if n.IsLeaf() {
		newid := NewID()
...
func (n *Node) hVSplit(i int, right bool) uint {
	if n.IsLeaf() {
		newid := NewID()
...
func (n *Node) HSplit(bottom bool) uint {
	if !n.IsLeaf() {
		return 0
	}
	if n.Kind == STUndef {
...
// of the current split
func (n *Node) VSplit(right bool) uint {
	if !n.IsLeaf() {
		return 0
	}
	if n.Kind == STUndef {
